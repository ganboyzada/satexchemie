    ...
    return {
      ...
      dsnScrollTop: function () {
        ...
        tl.to(
          ".scroll-to-top > img",
          1,
          { rotation: wrap.height() / 2, ease: easeAnimate },
          0
        );
        tl.to(
...
$(".btn-cart").click(function () {
  $(".auth-frame-login, .auth-frame-register").removeClass("active");
  $(".cart-frame").toggleClass("active");
});

$(".btn-login").click(function () {
  $(".cart-frame, .auth-frame-register").removeClass("active");
  $(".auth-frame-login").toggleClass("active");
});

$(".btn-register").click(function () {
  $(".cart-frame, .auth-frame-login").removeClass("active");
  $(".auth-frame-register").toggleClass("active");
});
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.message == "success") {
        $(".cart-content ul").children().remove();

        $(".cart-content ul").append(response.html);

        var cart_count = parseInt(response.count);

        $(".upper-right-header .btn-cart").attr("data-count", cart_count);
        animateCartCount();
        feather.replace();
      }
    },
...
function animateCartCount() {
  $(".btn-cart").addClass("animated");

  setTimeout(function () {
    $(".btn-cart").removeClass("animated");
  }, 400);
}
...
$(document).on("change", ".cart-item .quantity-adjust input", function (e) {
...
$(".btn-addtocart").on("click", function (e) {
  var form = $(this).closest(".addtocart-toolbar");
  e.preventDefault();
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.message == "success") {
        ...
        toastr.success(cart_success_msg);
      }
    },
    error: function (request, error, response) {
      toastr.error(
...
$(document).on("click", ".delete_cart_item", function () {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.message == "success") {
        toastr.success(cart_delete_success);
        setTimeout(function () {
          updateCart();
        }, 100);
      }
...
$(document).on("click", ".confirm_checkout", function (e) {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.message == "success") {
        toastr.success(checkout_success_msg);
        var order = response.item;
        $.ajax({
            type: "POST",
            url: "/add_order",
            data: {
...
$(document).on("click", ".quantity-adjust button", function (e) {
  e.preventDefault();
  ...
  if (parseInt(quantity.val()) > 0 || dq > 0) {
    quantity.val(parseInt(quantity.val()) + dq);
  }
});

$(document).on("click", ".cart-item .quantity-adjust button", function (e) {
